...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
        ...
        { "testinit" , "Test" , testinit},
        { "malloc", "MallocTest" , testMallocAndFree },
        { "free" , "FreeTest" , testFree}
};                                     
...
static void testinit()
{
    ...
    //3G 메모리를 할당받는다.
     memoryHead = kMemoryInit(0xC0000000);
     head = memoryHead;
     kPrintf("total size : %x\n",head->size);
...
//     nodet_t* head;
//     nodet_t* movedHead;
//     nodet_t* temp = memoryHead;

//     head = memoryHead;

//     while(head->next)
//     {
//         if(head->size < memorysize)
//             head = head->next;
//         else
//             break;
//     }
...
//             if(temp->next == head )
//         }
//         movedHead = (void*)head + memorysize + sizeof(nodet_t);
//         movedHead -> size = head->size - memorysize - sizeof(nodet_t);
//         movedHead -> next = NULL;

//         head->size = memorysize;
//         head->next = 0xFFFFFFFF;

        
//         memoryHead = movedHead;
//     }
// }
void *malloc(int memorysize)
{
    ...
    //head가 전체 chunk 메모리를 가르킨다.
    head = memoryHead;
    
    //할당하려는 메모리가 head가 가르키는 메모리보다 크다면 다음 free 리스트로 넘어간다.
    if(head->size < memorysize){
         while(head -> next){
             head = head -> next;
             if(head->size >= memorysize)
    ...
    //free 리스트에서 2번째 리스트 이후에 할당하는 경우
    if(head->next != NULL)
        movedHead = head-> next;
    //맨 끝 free 리스트에서 할당하는 경우
    else{
        movedHead = (void*)head + memorysize + sizeof(nodet_t);
        movedHead -> size = head->size - memorysize - sizeof(nodet_t);
        movedHead -> next = NULL;
    ...
    //할당하는 메모리의 헤더 할당
    head->size = memorysize;
    head->next = (nodet_t*)0xFFFFFFFF;

    //free 리스트 중간에서 할당 후 링크 연결
    if(((nodet_t*)memoryHead) ->next !=NULL && ((nodet_t*)memoryHead)->next != 0xFFFFFFFF)
    {
        temp = ((nodet_t*)memoryHead);
        while(temp->next){
            if(temp->next == head){
                temp-> next = movedHead;
                break;
    ...
    //free 리스트 첫번째 노드 할당 후 전체 포인터 변경
    else if(((nodet_t*)memoryHead)->next == 0xFFFFFFFF)
        memoryHead = movedHead;
    else
        memoryHead = (void*)(memoryHead + memorysize + sizeof(nodet_t));

  
    //할당한 메모리 주소 반환  
    return (void *)head+sizeof(nodet_t);
...
//malloc 명령어 
//파라미터로 할당할 byte 넘겨줌
void testMallocAndFree(const char * pcParameterBuffer)
{
    PARAMETERLIST stList;
    char vcType[ 30 ];
    void *p = NULL;
    ...
    int memorysize = kAToI(vcType,10);
...
{
    nodet_t *head;
    head = pointer - sizeof(nodet_t);

    //해제할 메모리를 free 리스트에 연결
    head -> next = memoryHead;
    memoryHead = (void*)head;
}


//free 명령어
void testFree(const char * pcParameterBuffer)
{
    PARAMETERLIST stList;
    char vcType[ 30 ];
    void *p = NULL;
    ...
    int memorysize = kAToI(vcType,16);

    free(memorysize);

    
    kPrintf("free success\n");
}