                  ...
                  QWORD qwEntryPointAddress )
{
    ...
    kSetUpTask( pstTask, qwFlags, qwEntryPointAddress, pvStackAddress, 
            TASK_STACKSIZE );
    // pstTask->ticketNum = kSetTicket(pstTask);
    //kPrintf("qwflag : %d\n",pstTask->qwFlags);
    globalTicket += kGetTicket((pstTask->qwFlags) & 0xf);   
...
static int kGetTicket(int bPriority){
    switch(bPriority){
        case 0:
            return 32;
        case 1:
...
static int kSetTicket(TCB* pstTCB){
    switch((pstTCB->qwFlags) & 0xf){
        case 0:
            return 32;
        case 1:
            return 16;
...
{
    TCB* pstTarget = NULL;
    int iTaskCount, i, j, k;
    int winner = random_generator() % globalTicket;
    int ticketCount = 0;
    ...
    for( j = 0 ; j < 2 ; j++ )
    {
        ...
        for( i = 0 ; i < TASK_MAXREADYLISTCOUNT ; i++ )
        {
            ...
            if(iTaskCount > 0){
                LIST* list = &(gs_stScheduler.vstReadyList[ i ]);
                LISTLINK* tmp = kGetHeaderFromList(list);
                for( k = 0 ; k < iTaskCount ; k++ )
                {
                    ticketCount += kGetTicket(i); 
                
                    if(winner <= ticketCount)
                    {
                    // pstTarget = ( TCB* ) kRemoveListFromHeader( &( gs_stScheduler.vstReadyList[ i ] ) );
                    // pstTarget = (TCB* )kRemoveListByTicket(tmp);
                        // globalTicket -= kGetTicket(i);
                        pstTarget = (TCB *)kRemoveList(&list,tmp->qwID);
                        // kPrintf("%d",tmp->qwID);
