...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
    {
        ...
        {"CREATE", "Create DB Table, ex) CREATE TABLE Student", kCreateDB},
        {"SELECT", "Select DB Row, ex) SELECT * FROM Student", kSelectDB},
        {"INSERT", "Insert DB Row, ex) INSERT INTO Student", kInsertDB},
        {"UPDATE", "Update DB Row, ex) UPDATE Student set Name = Jungeun WHERE Name = Jaeho", kUpdateDB},
        {"DELETE", "Delete DB Row, ex) DELETE FROM Student WHERE Name = Jungeun", kDeleteDB},
        {"DROP", "Drop DB Table, ex) DROP TABLE Studentn", kDropDB},
...
/**
 *  ,으로 구분된 파라미터의 내용과 길이를 반환
 */
int kGetNextColumn(PARAMETERLIST *pstList, char *pcParameter)
{
    int i;
    int iLength;
 ...
 *  ,으로 구분된 파라미터의 내용과 길이를 반환
 */
int kGetNextEnter(PARAMETERLIST *pstList, char *pcParameter)
{
    int i;
    int iLength;
    ...
    for (i = pstList->iCurrentPosition; i < pstList->iLength; i++)
    {
        if (pstList->pcBuffer[i] == KEY_ENTER)
        {
            break;
        }
    }
...
static void kCreateDB(const char *pcParameterBuffer)
{
    PARAMETERLIST stList;
    char fileName[10];
    char vcBuffer[10];
    ...
    int tmp = 0; // 파라미터를 추출
    kInitializeParameter(&stList, pcParameterBuffer);
    kGetNextParameter(&stList, vcBuffer);
    kGetNextParameter(&stList, vcDBname);
    tmp = kStrLen(vcDBname);
    vcDBname[tmp++] = '.';
    vcDBname[tmp++] = 'd';
    vcDBname[tmp++] = 'b';
    // kPrintf(vcDBname);
    if (kMemCmp("TABLE", vcBuffer, 5) == 0)
    {
        fp = fopen(vcDBname, "w");
        if (fp == NULL)
        {
        ...
        vcDBbuffer[idx++] = '\n';
        vcDBbuffer[idx] = '\0';
        if (fwrite(&vcDBbuffer, idx + 1, 1, fp) != 1)
        {
            kPrintf("File Wirte Fail\n");
            return;
        }
        kPrintf("%s Create Success\n", vcDBname);
        fclose(fp);
    }
    else
    {
        kPrintf("wrong syntax\n");
    }
}

static void kSelectDB(const char *pcParameterBuffer)
{
    PARAMETERLIST stList, fileList, columnList, rowList;
    char vcItem[100];
    char vcFROM[10];
    char vcDBName[20];
    char vcWHERE[10];
    char vcCondition[100];
    FILE *fp;
    BOOL where = FALSE;
    ...
    char column[100][100];
    char rowBuffer[100][100];
    char row[100][100][100];
    char printBuffer[500];

    BYTE bKey;

    kInitializeParameter(&stList, pcParameterBuffer);
    kGetNextParameter(&stList, vcItem);
    kGetNextParameter(&stList, vcFROM);
    if (kMemCmp(vcFROM, "FROM", 5) != 0)
    {
        kPrintf("Wrong SELECT Statement!\n");
        return;
    }

    iLength = kGetNextParameter(&stList, vcDBName);
    vcDBName[iLength++] = '.';
    vcDBName[iLength++] = 'd';
    vcDBName[iLength++] = 'b';
    vcDBName[iLength] = '\0';
    if ((iLength - 3 > (FILESYSTEM_MAXFILENAMELENGTH - 1)) || (iLength == 0))
    {
        kPrintf("Too Long or Too Short TABLE Name\n");
        return;
    }

    if( kGetNextParameter(&stList, vcWHERE) != 0 ){
        if( kMemCmp( vcWHERE, "WHERE", 6) != 0)
        {
            kPrintf("Wrong SELECT Statement!\n");
            return ;
        }
    ...
    if ( where ){
        kGetNextParameter( &stList, vcCondition );
    }
    ...
    while(1){
        ...
        if ( bKey == '\0' ){
            fileBuffer[bufferCount++] = bKey;
            break;
        }
        fileBuffer[bufferCount++] = bKey;
    }
    fclose(fp);
    // bufferCount = 4 ; ex) buffer = 'abc\0'

    // rowBuffer and rolumnBuffer
    kInitializeParameter( &fileList, fileBuffer );
    kGetNextEnter( &fileList, columnBuffer );
    while( kGetNextEnter( &fileList, rowBuffer[rowCount] ) != 0 ){
        rowCount++;
    }


    kInitializeParameter( &columnList, columnBuffer );
    while( ( tmpLength = kGetNextColumn( &columnList, column[columnCount] ) ) != 0 ){
        columnCount++;
    ...
    for ( int i = 0 ; i < rowCount ; i ++ ) {
        kInitializeParameter( &rowList, rowBuffer[ i ] );
        for ( int j = 0 ; j < columnCount ; j ++ ){
            tmpLength = kGetNextColumn( &rowList, row[i][j] );
            if (tmpLength > Longest){
            Longest = tmpLength;
            }
        }
...
///////////////////////////////////////////////////////
    ...
    for( int i = 0; i < columnCount ; i++ ){
        printBuffer[printCount++] = '|';
        tmpLength = kStrLen( column[ i ] );
        kMemCpy( printBuffer + printCount, column[i], tmpLength );
        printCount += tmpLength;
        for( int  j = 0 ; j < Longest - tmpLength ; j ++ ){
            printBuffer[printCount++] = ' ';
        }
    }
    printBuffer[printCount++] = '|';
    ...
    for (int k = 0; k < rowCount; k++)
    {
        ...
        {
            printBuffer[printCount++] = '|';
            tmpLength = kStrLen(row[k][i]);
            kMemCpy(printBuffer + printCount, row[k][i], tmpLength);
            printCount += tmpLength;
            for (int j = 0; j < Longest - tmpLength; j++)
            {
                printBuffer[printCount++] = ' ';
    ...
    printBuffer[printCount++] = '\0';

    kPrintf("%s", printBuffer);
    ////////////////////////////////////////////////////
...
static void kInsertDB(const char *pcParameterBuffer)
{
    PARAMETERLIST stList, columnList, valueList;
    char vcINTO[5];
    char vcVALUES[7];
    char vcFileName[50];
    char vcColumn[100];
    ...
    kInitializeParameter(&stList, pcParameterBuffer);
    kGetNextParameter(&stList, vcINTO);
    if (kMemCmp(vcINTO, "INTO", 5) != 0)
    {
        kPrintf("Wrong INSERT Statement!\n");
        return;
    }
    iLength = kGetNextParameter(&stList, vcFileName);
    vcFileName[iLength++] = '.';
    vcFileName[iLength++] = 'd';
    vcFileName[iLength++] = 'b';
    vcFileName[iLength] = '\0';
    if ((iLength - 3 > (FILESYSTEM_MAXFILENAMELENGTH - 1)) || (iLength == 0))
    {
        kPrintf("Too Long or Too Short TABLE Name\n");
    ...
    kGetNextParameter(&stList, vcVALUES);
    if (kMemCmp(vcVALUES, "VALUES", 7) != 0)
    {
        kPrintf("Wrong INSERT Statement!\n");
        return;
    }
    iLength = kGetNextParameter(&stList, vcValue);
    if (vcValue[0] != '(' || vcValue[iLength - 1] != ')')
    {
        kPrintf("Wrong INSERT Statement!\n");
        return;
    }
    kMemCpy(vcValue, vcValue + 1, iLength - 1);
    vcValue[iLength - 2] = '\n';
    vcValue[iLength - 1] = '\0';
    fp = fopen(vcFileName, "r+");
    if (fp == NULL)
    ...
    while (1)
    {
        ...
        if (bKey == KEY_ENTER)
        {
            vcColumn[iColumnCount] = '\0';
            break;
        }
        vcColumn[iColumnCount++] = bKey;
    }
    iColumnCount = 0;
    kInitializeParameter(&columnList, vcColumn);
    while (kGetNextColumn(&columnList, columns[iColumnCount]) != 0)
    {
        iColumnCount++;
    }

    iValueCount = 0;
    kInitializeParameter(&valueList, vcValue);
    while (kGetNextColumn(&valueList, values[iValueCount]) != 0)
    {
        iValueCount++;
    }
    iValueCount = 0;
    ...
    if (fwrite(&vcValue, iLength, 1, fp) != 1)
    {
    ...
    kPrintf("Query executed successfully: INSERT %s\n", pcParameterBuffer);
...
static void kDropDB(const char *pcParameterBuffer)
{
    PARAMETERLIST stList;
    char vcBuffer[10];
    char vcDBname[10];
    int iLength;
    kInitializeParameter(&stList, pcParameterBuffer);
    iLength = kGetNextParameter(&stList, vcBuffer);
    vcBuffer[iLength] = '\0';
    if (kMemCmp("TABLE", vcBuffer, 5) != 0)
    {
    ...
    if ((iLength > (FILESYSTEM_MAXFILENAMELENGTH - 1)) || (iLength == 0))
    {
        kPrintf("Too Long or Too Short DBFile Name\n");
        return;
    }
    if (remove(vcDBname) != 0)
    {
        kPrintf("DBFile Not Found or File Opened\n");
        return;
    }
    kPrintf("DBFile Delete Success\n");
}

static void kUpdateDB(const char *pcParameterBuffer)
{
}

static void kDeleteDB(const char *pcParameterBuffer)
{
    PARAMETERLIST stList, fileList, columnList, rowList;
    char vcFROM[10];
    char vcDBName[20];
    ...
    vcDBName[iLength++] = 'b';
    vcDBName[iLength++] = '\0';
    kMemCpy(DBName,vcDBName,iLength);
    ...
    iLength = kGetNextParameter(&stList, vcWHERE);
    vcWHERE[iLength]='\0';
    if( kMemCmp( vcWHERE, "WHERE", 6) != 0)
    {
        kPrintf("Wrong SELECT Statement!\n");
    ...
    iLength = kGetNextParameter( &stList, vcCondition );
    vcCondition[iLength] = '\0';
    ...
    for(int i=0;i<kStrLen(vcCondition);i++){
        if(vcCondition[i]=='='){
            condAttr[i] = '\0';
            attrLen = i;
            attribute = FALSE;
        ...
        if(attribute){
            condAttr[i] = vcCondition[i];
        }
        else{
            condValue[i-attrLen-1] = vcCondition[i];
        }
    }
    int i;
    int j;
    for(i=0;i<columnCount;i++){
        if(kMemCmp(column[i],condAttr,kStrLen(condAttr))==0){
            break;
        }
    }
    for(j=0;j<rowCount;j++){
        if(kMemCmp(row[j][i],condValue,kStrLen(condValue))==0){
            break;
        }
    }
    ...
    for(int p=0;p<columnCount;p++){
        for(int q=0;q<kStrLen(column[p]);q++){
            final[finidx++] = column[p][q];
            // kPrintf("%s",column[p]);
        }
        if(p!=columnCount-1){
            final[finidx++] = ',';
    ...
    for(int k=0;k<rowCount;k++){
        ...
        for(int p=0;p<columnCount;p++){
            for(int q=0;q<kStrLen(row[k][p]);q++){
                final[finidx++] = row[k][p][q];
                // kPrintf("%s",column[p]);
            }
            if(p!=columnCount-1){
                final[finidx++] = ',';
    ...
    final[finidx++] = '\0';
    if (fwrite(final, finidx, 1, fp) != 1)
    {
        kPrintf("File Write Fail\n");
        return;
    }
    kPrintf("Delete Row Success\n");
